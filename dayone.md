...
There are a number of fixes, some better than others. The standard fix:
```
class Button {
    constructor(text, name) {
        this.name = name;
...
>Remember, you are creating a **new copy** of the method so that it can still use the `this` you want it to. If a method will never be called in the context of another object, there's no need to bind `this`.

# Arrow functions
Details are here: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions. Upshot: arrow functions ignore the `this` context of where they are invoked, instead they will have the `this` context from where they are defined. In a class, this is what's happening:
```
let _this = this;
...
- 1) It does work
- 2) It's usually a bad idea
- 3) One should try to avoid it
